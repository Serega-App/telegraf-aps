...
public class LiteMode {
    ...
    private static int disableAutoplay(int flags) {
        return flags & ~FLAG_AUTOPLAY_VIDEOS & ~FLAG_AUTOPLAY_GIFS;
    }
    ...
    public static void updatePresets(TLRPC.TL_jsonObject json) {
        for (int i = 0; i < json.value.size(); ++i) {
            ...
            if ("settings_mask".equals(kv.key) && kv.value instanceof TLRPC.TL_jsonArray) {
                ...
                try {
                    PRESET_LOW = disableAutoplay((int) ((TLRPC.TL_jsonNumber) array.get(0)).value);
                    PRESET_MEDIUM = disableAutoplay((int) ((TLRPC.TL_jsonNumber) array.get(1)).value);
                    PRESET_HIGH = disableAutoplay((int) ((TLRPC.TL_jsonNumber) array.get(2)).value);
                } catch (Exception e) {
                    FileLog.e(e);
                }
    ...
    public static void loadPreference() {
        ...
        int prevValue = value;
        value = disableAutoplay(preferences.getInt("lite_mode6", defaultValue));
        if (loaded) {
