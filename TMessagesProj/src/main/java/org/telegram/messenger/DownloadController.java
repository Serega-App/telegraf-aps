...
public class DownloadController extends BaseController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public DownloadController(int instance) {
        ...
        keepOnlyPhotoAndStories(lowPreset);
        keepOnlyPhotoAndStories(mediumPreset);
        keepOnlyPhotoAndStories(highPreset);
        keepOnlyPhotoAndStories(mobilePreset);
        keepOnlyPhotoAndStories(wifiPreset);
        keepOnlyPhotoAndStories(roamingPreset);
    ...
    private void keepOnlyPhotoAndStories(Preset preset) {
        if (preset == null) {
            return;
        }
        for (int a = 0; a < preset.mask.length; a++) {
            preset.mask[a] = AUTODOWNLOAD_TYPE_PHOTO;
        }
        preset.preloadStories = true;
    }
        ...
        getConnectionsManager().sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
            ...
            if (response != null) {
                ...
                for (int a = 0; a < 3; a++) {
                    ...
                    keepOnlyPhotoAndStories(preset);
                }
                keepOnlyPhotoAndStories(lowPreset);
                keepOnlyPhotoAndStories(mediumPreset);
                keepOnlyPhotoAndStories(highPreset);
